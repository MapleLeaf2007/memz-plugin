...
export class Unicode extends plugin {
  constructor() {
    super({
      ...
      priority: 6,
      rule: [
        { reg: /^(#?)(unicode|ascii|punycode)(编码|解码)\s*(.+)/, fnc: 'handleEncodingDecoding' }
      ]
    });
  }
  ...
  async handleReply(e, handler) {
    ...
    try {
      if (operation === 'unicode') {
        result = action === '编码' ? encodeToUnicode(input) : decodeFromUnicode(input);
      } else if (operation === 'ascii') {
        result = action === '编码' ? encodeToAscii(input) : decodeFromAscii(input);
      } else if (operation === 'punycode') {
        result = action === '编码' ? encodeToPunycode(input) : decodeFromPunycode(input);
      }
  ...
  async handleEncodingDecoding(e) {
    ...
    await this.handleReply(e, { reg: /^(#?)(unicode|ascii|punycode)(编码|解码)\s*(.+)/, fn: this.handleReply });
  }
}
