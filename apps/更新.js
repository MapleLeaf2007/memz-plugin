...
export class Update extends plugin {
    ...
    async runUpdate(isForce) {
        ...
        if (ret.error) {
            logger.mark(`${this.e.logFnc} 更新失败：memz-plugin插件`)
            this.gitErr(ret.error, ret.stdout)
            return false
        }
        /** 获取插件提交的最新时间 */
        let time = await this.getTime('memz-plugin')
        ...
        if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
            ...
            this.isUp = true
            /** 获取fengye-plugin的更新日志 */
            let log = await this.getLog('memz-plugin')
    ...
    async getLog(plugin = '') {
        // 执行命令，获取插件日志
        let cm = `cd ./plugins/${plugin}/ && git log  -50 --oneline --pretty=format:"%h||[%cd]  %s" --date=format:"%F %T"`
        let logAll
        try {
    ...
    async getLog(plugin = '') {
        let cm = `cd ./plugins/memz-plugin/ && git log  -80 --oneline --pretty=format:"%h||[%cd]  %s" --date=format:"%F %T"`
        let logAll
        try {
