...
export class Game extends plugin {
    ...
    async applyGame(e) {
        ...
        try {
            ...
            if (currentCount >= maxApplications) {
                e.reply(`您已经达到申请游戏的上限（${maxApplications}个），请先取消一些申请后再继续。`, true);
                return;
            }
    ...
    async listMyApplications(e) {
        ...
        try {
            ...
            for (const gameId of gameIds) {
                const application = await redis.hget('GameApplications', `${userId}:${gameId}`);
                if (application) {
                    const parsedApp = JSON.parse(application);
                    applicationDetails.push(`游戏ID: ${parsedApp.gameId}, 申请时间: ${parsedApp.timestamp}`);
                }
            }

            const response = applicationDetails.length > 0
    ...
    async listAllApplications(e) {
        try {
            ...
            const applicationDetails = applicationArray.map(app => {
