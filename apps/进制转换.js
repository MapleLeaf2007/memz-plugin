...
const { BaseConversionAll } = Config.getYaml('config', 'memz-config');
...
export class TextMsg extends plugin {
  ...
  async BaseConversion(e) {
    ...
    let args = e.msg.match(/#?进制转换\s*(.+)/)[1].trim().split(/\s+/);
    ...
    try {
      const convertedNumber = await convertBase(number, parseInt(fromBase, 10), parseInt(toBase, 10));
      await e.reply(`原始数 ${number} 起始进制 ${fromBase} 目标进制 ${toBase} 是 ${convertedNumber}`);
    } catch (error) {
